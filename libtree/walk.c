...
static void 
walkv(void *sink, const Stk *parent_unacc, const Stk *parent_flags, 
      Stk *unacceptable, Stk *unacceptable_flags, Stk *deferred, Stk *deferred_flags)
{
    Key_t key;
    hcell_type type;
    int n;
...
static void Walkbody(int commonlev, int bodylev){
	...
	ABMPoll(&Abm);
	walkv(sink, &unacc[l], &unacc_flags[l], &unacc[l+1], &unacc_flags[l+1], &def1, &def1_flags);
	deffrom = &def1;
	...
	while( StkSz(deffrom) ){
	    ...
	    walkv(sink, &arrived, &arrived_flags, &unacc[l+1], &unacc_flags[l+1], defto, defto_flags);
	    deftmp = deffrom;
	    deffrom = defto;
	    defto = deftmp;
...
void Walk(tree_t *srctp, tree_t *sinktp, int sinksz,
	  walkinit_t WalkInitSrc, macv_t MAC, inherit_t InheritSink){
...
void
WalkInit(tree_t *srctp, tree_t *sinktp, int sinksz, 
	 walkinit_t WalkInit, macv_t MAC, inherit_t InheritSink)
{
