...
main(int argc, char **argv)
{
    MPMY_Init(&argc, &argv);
    if (argc != 9) {
	singlPrintf("usage: %s [xyz]_min [xyz]_max infile.sdf outbase\n", argv[0]);
	singlPrintf("usage: (min max in Mpc/h)\n", argv[0]);
	exit(1);
    }
    ...
    float x_min = atof(argv[1]);
    float y_min = atof(argv[2]);
    float z_min = atof(argv[3]);
    float x_max = atof(argv[4]);
    float y_max = atof(argv[5]);
    float z_max = atof(argv[6]);
    char *infile = argv[7];
    char *outfile = argv[8];
    ...
    for (int i = 0; i < nobj; i++) {
	body *b = &btab[i];
	if (b->pos[0] < 100.0*x_min/h || b->pos[1] < 100.0*y_min/h || b->pos[2] < 100.0*z_min/h) continue;
	if (b->pos[0] >= 100.0*x_max/h || b->pos[1] >= 100.0*y_max/h || b->pos[2] >= 100.0*z_max/h) continue;
	btab[new_nobj++] = *b;
    }
    gnobj = nobj = new_nobj;
    btab = Realloc(btab, (nobj+1) * sizeof(body)); /* don't free if new_nobj==0 */
