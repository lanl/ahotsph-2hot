...
#define MAXCOUNTINDEX (1.75*Nmesh) /* sqrt(3) */
#define EPS 3.0e-6
...
void
do_pspec(int Nmesh, float L0mpc, float chain_factor, float fft_kmax, fftwf_complex *d, int *counts, double *sum, double *linear_sum, double *sharpen_sum, double *sum4, float *Ktab, float *Tftab, float *D2, int Ntable)
{
    int i, j, k;
    int ii, jj, kk;
    float kindex;
    ...
    float wavenum;
    int index;
    int parity_i, parity_ij, parity_ijk;
    float sharpen;
    float logps2, ps2;
	...
	for (k = 0; k < Nmesh; k++) {
	  ...
	  if (i <= Nmesh/2) ii = i; else ii = i-Nmesh;
	  if (j <= Nmesh/2) jj = j; else jj = j-Nmesh;
	  if (k <= Nmesh/2) kk = k; else kk = k-Nmesh;
	  parity_i = (ii) ? ii : 1;
	  parity_ij = parity_i * (jj) ? jj : 1;
	  parity_ijk = parity_ij * (kk) ? kk : 1;
	  
	  kindex = sqrt(ii*ii+jj*jj+kk*kk);
	  if (kindex == 0.0) continue;
	  wavenum = 2.0*M_PI*chain_factor*kindex/L0mpc;
	  splint(sampling_x, sampling_corr, sampling_D2, SAMPLING_N, 
		 kindex/Nmesh, &sharpen);
	  splint(Ktab, Tftab, D2, Ntable, log(wavenum), &logps2);
	  ps2 = exp(logps2);
	  real = creal(d[idx(i,j,k)]);
	  imag = cimag(d[idx(i,j,k)]);
	  mag = real*real+imag*imag;
	  index = (int)(2.0*kindex+1.0);
	  if (index >= MAXCOUNTINDEX) Error("index is %d\n", (int)index);
	  if (kindex < fft_kmax) {
	    counts[index]++;
	    sum[index] += mag;
	    linear_sum[index] += ps2;
	    sharpen_sum[index] += 1.0/sharpen;
	    sum4[index] += (real*real*real*real+imag*imag*imag*imag)/(ps2*ps2);
	  }
	}
      }
    }
    ...
    MPMY_Combine(sum4, sum4, MAXCOUNTINDEX, MPMY_DOUBLE, MPMY_SUM);
}
...
main(int argc, char **argv)
{
    ...
    int fileversion;
    float igf;
    int write_modes, write_modes_complex;
    ...
    int *counts;
    double *sum;
    double *linear_sum, *sharpen_sum, *sum4;
	...
	SDFgetdoubleOrDie(sdfp, "growthfac",  &cosmo.growthfac);
...
#if 0
    minarr = Malloc(MPMY_Nproc() * sizeof(float));
    maxarr = Malloc(MPMY_Nproc() * sizeof(float));
    MPMY_Gather(&xmin, 1, MPMY_FLOAT, minarr, 0);
    MPMY_Gather(&xmax, 1, MPMY_FLOAT, maxarr, 0);
    for (i = 0; i < MPMY_Nproc(); i++) {
      singlPrintf("%d %14.8g %14.8g\n", i, minarr[i], maxarr[i]);
    }
    Free(minarr);
    Free(maxarr);
#endif
    ...
    counts = Calloc(Nmesh*2, sizeof(int));
    sum = Calloc(Nmesh*2, sizeof(double));
    linear_sum = Calloc(Nmesh*2, sizeof(double));
    sharpen_sum = Calloc(Nmesh*2, sizeof(double));
    sum4 = Calloc(Nmesh*2, sizeof(double));

    do_pspec(Nmesh, L0mpc, chain_factor, fft_kmax, d, counts, sum, linear_sum, sharpen_sum, sum4, 
	     Ktab, Tftab, D2, Ntable);

    if (MPMY_Procnum() == 0) {
	sprintf(name, "%s.%dc%.0f", outname, Nmesh, chain_factor);
	Fopen(outfile, name, "w");

	fprintf(outfile, "# nsamples = %ld %s mean_mass = %g Nmesh = %d chain_factor = %.0f\n", nsamples, (mass_weighted) ? "mass_weighted" : "", M0/nsamples, Nmesh, chain_factor);
	fprintf(outfile, "# nobj R0 igf Nmesh A_s interp_method h_100\n");
	fprintf(outfile, "%ld %12.8g %12.8g %d %12.8g %d %12.8g\n", gnobj, R0, igf, Nmesh, A_s, interp_method, cosmo.h_100);
	fprintf(outfile, "# k avg_mag linear_mag shot_noise sharpen count\n");
	for (i = 1; i < Nmesh; i++) {
	    if (counts[i] > 1) {
		fprintf(outfile, "%12.8g %12.8g %12.8g %12.8g %12.8g %8d\n", 
			M_PI*chain_factor*i/L0mpc, 
			L0mpc*L0mpc*L0mpc*sum[i]/(counts[i]), 
			linear_sum[i]/counts[i], 
			L0mpc*L0mpc*L0mpc/nsamples, 
