...
INLINE Key_t KeyOrInt(Key_t u, unsigned int i) {
    Key_t ret;
    ret.k[0] = u.k[0] | i;
    return (ret);
}
...
INLINE unsigned int KeyAndInt(Key_t u, unsigned int i) { return (u.k[0] & i); }
...
INLINE int TreeLevel(Key_t key, int ndim) {
    int level;
...
INLINE int CommonLev(Key_t bkey1, Key_t bkey2, int ndim) {
...
INLINE int KeyContained(Key_t outer, Key_t key, int ndim) {
    int ret;
    int difference;
