...
void MPMY_CheckpointSetup(int job_seconds, int interval_seconds, int step_seconds) {
    if (MPMY_Procnum() == 0) {
        ...
        if (job_seconds == -1)
            ...
            job_end = job_start + job_seconds - step_seconds;
...
int MPMY_CheckpointDue(int next_output_seconds) {
    ...
    if (MPMY_Procnum() == 0) {
        t = time(NULL);
        if (t >= checkpoint_next)
            retval = 1;
        if (job_end > 0 && t >= job_end)
            retval = 1;
    }
    if (retval)
        Msg_do("Checkpoint Due\n");
...
void MPMY_CheckpointFinished(void) {
    ...
    if (MPMY_Procnum() == 0) {
        ...
        if (job_end > 0 && checkpoint_next > job_end)
            checkpoint_next = job_end;
...
double MPMY_Wtime(void) { return clock() / CLOCKS_PER_SEC; }
#endif

#ifndef HAVE_MPMY_JOBREMAINING
int MPMY_JobRemaining(void) { return -1; }
#endif
