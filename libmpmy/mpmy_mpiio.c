...
int MPMY_Fclose(MPMYFile *Fp) {
    ...
    if (fp->async) {
        ...
        if (cnt != fp->async_nwrite)
            Error("async MPMY_Fwrite has a problem, wrote %d of %d\n", cnt, fp->async_nwrite);
    }
...
size_t MPMY_Fread(void *ptr, size_t size, size_t nitems, MPMYFile *Fp) {
    ...
    if (fp->iomode == MPMY_SINGL) {
        if (nread >= (1 << 31))
            Error("MPMY_SINGL does not yet support large reads\n");
        ...
        for (i = 0; i < MPMY_Procnum(); i++) { mpi_offset += sizes[i]; }
...
size_t MPMY_Fwrite(const void *ptr, size_t size, size_t nitems, MPMYFile *Fp) {
    ...
    if (fp->iomode == MPMY_SINGL) {
        if (nwrite >= (1 << 31))
            Error("MPMY_SINGL does not yet support large writes\n");
        MPI_File_write_all(fp->fd, (void *)p, nwrite, MPI_CHAR, &status);
        MPI_Get_count(&status, MPI_CHAR, &cnt);
        if (cnt != nwrite)
            Error("MPMY_Fwrite has a problem, wrote %d of %ld\n", cnt, nwrite);
    } else {
        MPI_Offset mpi_offset;
        ...
        while (write_cycles--) {
            ...
            if (fp->async) {
                if (nwrite == MAXIOSIZE)
                    Error("Async writes not yet supported for chunks larger than MAXIOSIZE\n");
