...
static int fseekrd_nfile(int fd, off_t offset, int whence, void *buf, int reclen, int nrecs) {
    off_t offend;
    ...
    for (i = 0; i < nproc; i++) {
        if (allbuf[i].start <= offset && offset < allbuf[i].start + allbuf[i].flen) {
            ...
            if (nreq >= MAXREQ)
                Error("Too many Irecvs in fseekrd\n");
            ...
            if (nreq >= MAXREQ)
                Error("Too many Irecvs in fseekrd\n");
            MPMY_Irecv((char *)buf + reclen * nrecs - size, size, i, MPMY_IOTAG + 1, &req[nreq]);
            ++nreq;
        }
    }
...
static int fseekrd(int fd, off_t offset, int whence, void *buf, int reclen, int nrecs) {
    int doseek;
    int real_whence;
    off_t nread = 0;
    off_t len;
    ...
    if (doseek) {
        ...
        if (lseek(fd, offset, real_whence) == -1) {
            Error(
                "fseekrd: lseek(%d, %ld, %d) failed, errno=%d\n", fd, (long)offset, whence, errno);
            return -1;
        }
    }
    len = 0;
    while (len < reclen * nrecs) {
        ...
        if (nread == -1) {
            Error(
                "fseekrd: read(%d, %ld) failed, errno=%d\n", fd, (long)reclen * nrecs - len, errno);
            return -1;
        } else if (nread == 0) {
...
size_t MPMY_Fseekrd(
    MPMYFile *Fp, off_t offset, int whence, void *buf, size_t reclen, size_t nrecs) {
    struct _File *fp = (struct _File *)Fp;
