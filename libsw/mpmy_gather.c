...
int MPMY_Alltoall(void *sendbuf,
                  int sendcount,
    ...
    for (relative = 0; relative < 1 << doc_procs; relative++) {
        ...
        Msgf(("local Alltoall MPMY_Shift(dest=%d, instart=%p, incnt=%d, outstart=%p, outcnt=%d)\n",
              dest,
              recvbuf + dest * recvbytes,
              recvbytes,
              sendbuf + dest * sendbytes,
              sendbytes));
        MPMY_Shift(dest,
                   recvbuf + dest * recvbytes,
                   recvbytes,
            ...
            if (MPMY_Procnum() != my_leader) {
                ...
                if (MPMY_Procnum() == my_leader + proc) {
                    Msgf(("Alltoall relay recv from %d (source=%d, incnt=%d)\n",
                          my_leader,
                          dest_leader,
                          PROCS_PER_NODE * recvbytes));
                    MPMY_Irecv(recvbuf + dest_leader * recvbytes,
                               PROCS_PER_NODE * recvbytes,
                               my_leader,
                               ALLTOALL_TAG,
                               &req);
                    ...
                    if (ret != PROCS_PER_NODE * recvbytes) {
                        Error(
                            "Irecv failed, expected %d got %d\n", PROCS_PER_NODE * recvbytes, ret);
                    }
                }
            } else {
                memcpy(tmpbuf_s, sendbuf + dest * sendbytes, sendbytes);
                for (i = 1; i < PROCS_PER_NODE; i++) {
                    if (my_leader + i >= MPMY_Nproc())
                        break;
                    MPMY_Irecv(
                        tmpbuf_s + i * recvbytes, recvbytes, my_leader + i, ALLTOALL_RTAG, &req);
                    MPMY_Wait(req, &stat);
                    ret = MPMY_Count(&stat);
                    if (ret != recvbytes) {
                ...
                Msgf(("Alltoall relay for %d MPMY_Shift(dest=%d, incnt=%d, outcnt=%d)\n",
                      ...
                      dest_leader,
                      PROCS_PER_NODE * recvbytes,
                      PROCS_PER_NODE * sendbytes));
                MPMY_Shift(dest_leader,
                           tmpbuf_r,
                           PROCS_PER_NODE * recvbytes,
                           tmpbuf_s,
                           PROCS_PER_NODE * sendbytes,
                ...
                if (proc == 0) {
                    ...
                    Msgf(("Alltoall relay send to %d (source=%d, incnt=%d)\n",
                          ...
                          PROCS_PER_NODE * recvbytes));
                    MPMY_Isend(
                        tmpbuf_r, PROCS_PER_NODE * recvbytes, my_leader + proc, ALLTOALL_TAG, &req);
                    MPMY_Wait(req, 0);
                }
            }
        }
...
int MPMY_Alltoallv(void *sendbuf,
                   int *scount,
                   int *soff,
                   MPMY_Datatype sendtype,
                   void *recvbuf,
    ...
    for (relative = 0; relative < 1 << doc_procs; relative++) {
        ...
        Msgf(("local Alltoallv MPMY_Shift(dest=%d, instart=%p, incnt=%d, outstart=%p, outcnt=%d)\n",
              dest,
              recvbuf + roff[dest] * rsize,
              rcount[dest] * rsize,
              sendbuf + soff[dest] * ssize,
              scount[dest] * ssize));
        MPMY_Shift(dest,
                   recvbuf + roff[dest] * rsize,
                   rcount[dest] * rsize,
    ...
    for (relative = 1; relative < 1 << doc_nodes; relative++) {
        for (proc = 0; proc < PROCS_PER_NODE; proc++) {
            ...
            if (MPMY_Procnum() != my_leader) {
                ...
                if (scount[dest]) {
                    MPMY_Isend(sendbuf + soff[dest] * ssize,
                               scount[dest] * ssize,
                               my_leader,
                               ALLTOALL_RTAG,
                               &req);
                ...
                if ((MPMY_Procnum() == my_leader + proc) && recvbytes) {
                    Msgf(("Alltoallv relay recv from %d (source=%d, incnt=%d)\n",
                          ...
                          dest_leader,
                          recvbytes));
                    MPMY_Irecv(recvbuf + roff[dest_leader] * rsize,
                               recvbytes,
                               my_leader,
                               ALLTOALL_TAG,
                               &req);
                ...
                if (sendbytes || recvbytes) {
                    ...
                    for (i = 1; i < PROCS_PER_NODE; i++) {
                        if (my_leader + i >= MPMY_Nproc())
                            break;
                        if (stcnt[i]) {
                            MPMY_Irecv(tmpbuf_s + stoff[i] * ssize,
                                       stcnt[i] * ssize,
                                       my_leader + i,
                                       ALLTOALL_RTAG,
                                       &req);
...
int MPMY_Alltoallv_simple(void *sendbuf,
                          int *scount,
                          int *soff,
                          MPMY_Datatype sendtype,
                          void *recvbuf,
    ...
    for (relative = 1; relative < 1 << doc; relative++) {
        ...
        Msgf(("Alltoallv MPMY_Shift(dest=%d, instart=%p, incnt=%d, outstart=%p, outcnt=%d)\n",
              i,
              recvbuf + roff[i] * rsize,
              rcount[i] * rsize,
              sendbuf + soff[i] * ssize,
              scount[i] * ssize));
        MPMY_Shift(i,
                   recvbuf + roff[i] * rsize,
                   rcount[i] * rsize,
