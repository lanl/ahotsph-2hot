...
static void SDFwrite_alist64(const char *filename,
                             int mode,
                             int64_t gnobj,
                             int64_t nobj,
                             const void *btab,
...
void SDFunsetwroteheader(void) { wrote_header = 0; }

void SDFsetwroteheader(void) { wrote_header = 1; }
...
static void SDFwrite_alist64(const char *filename,
    ...
    if (MPMY_Procnum() == 0 && wrote_header == 0) {
        ...
        outstr(line);
        /* Need to be very careful with types sent to this function.  There is no type checking for
         * va_args */
        ...
        if (write_sha1) {
            ...
            memcpy(
                buf + header_len + MPMY_Nproc() * sizeof(struct md_s), btab, (size_t)bsize * nobj);
            Free(mdtab);
        } else {
...
retry:
    ...
    if (!(mode & MPMY_TRUNC))
        MPMY_Fseek(myfd, 0L, MPMY_SEEK_END);
