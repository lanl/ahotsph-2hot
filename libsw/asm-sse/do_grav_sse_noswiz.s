...
# parameter 7(eps2p): 32 + %ebp
# parameter 8(ncut): 36 + %ebp
	.globl   do_grav_sse_noswiz
do_grav_sse_noswiz:
.init:
        pushl     %ebp
        movl      %esp, %ebp
...
.loop:					# 112 flops in 43 cycles
					...
					# 9 mul
					# 1 rsqrt
	movaps	  (%eax), %xmm0		# mass
        movaps    16(%eax), %xmm5	# x
        movaps    32(%eax), %xmm6	# y
        movaps    48(%eax), %xmm7	# z
	subps	  (%esp), %xmm5
	subps	  16(%esp), %xmm6
	subps	  32(%esp), %xmm7
	movaps	  %xmm5, %xmm1
