...
static void process(int count, int tag) {
    int dest = inbuf[0];
    ...
    if (dest != MPMY_Procnum()) { /* relay */
        ...
        if (localid)
            ...
            /* local done message */
            if (src >= MPMY_ProcsPerNode())
                Error("Bad src in Poll()\n");
            if (localpoll[src])
                Error("localpoll[%d] already set!\n", src);
            if (localdone >= MPMY_ProcsPerNode())
                Error("localdone already acheived\n");
            localpoll[src] = 1;
            localdone++;
            ...
            if (remotedone >= nremote)
                Error("remotedone already acheived\n");
            remotepoll[src] = 1;
            if (++remotedone >= nremote)
                polldone = 1;
...
void PollSetup(void put(void *buf, int size), int max_size, int tag) {
    ...
    if (procs_per_node > MAXLOCAL)
        SinglError("MAXLOCAL too small\n");
    if ((MPMY_Nproc() - 1) / procs_per_node >= MAXREMOTE)
        SinglError("MAXREMOTE too small\n");
    ...
    if (localid == 0) {
        for (i = 0; i < MAXRELAY; i++) { relaybuf[i] = Malloc(INBUFSZ); }
    }
...
void Poll(int tag) {
    ...
    Msgf(("P(tag=%d)\n", tag));
    MPMY_Flick();
    while (MPMY_Test(inreq, &flag, &stat), flag) { process(stat.count, tag); }
}
...
void PollWait(MPMY_Comm_request req, int tag) {
    ...
    while (1) {
        if (MPMY_Test(req, &flag, 0), flag)
            return;
        if (MPMY_Test(inreq, &flag, &stat), flag)
...
void PollUntilDone(int tag) {
    ...
    if (localid == 0) {
        for (i = 0; i < MAXRELAY; i++) { Free(relaybuf[i]); }
    }
